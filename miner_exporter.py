...
SCRAPE_TIME = prometheus_client.Summary(
    "validator_scrape_time", "Time spent collecting miner data"
)
...
def stats():
    ...
    try:
        ...
        if docker_container is None:
            log.error(
                f"docker failed while bootstrapping. Not exporting anything. Error: {ex}"
            )
            return
...
def collect_hbbft_performance(docker_container, miner_name):
    ...
    for line in out.output.decode("utf-8").split("\n"):
        ...
        HBBFT_PERF.labels("hbbft_perf", "Penalty", miner_name).set(
            hval.get("pen_val", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "BBA_Total", miner_name).set(
            hval.get("bba_tot", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "BBA_Votes", miner_name).set(
            hval.get("bba_votes", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "Seen_Total", miner_name).set(
            hval.get("seen_tot", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "Seen_Votes", miner_name).set(
            hval.get("seen_votes", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "BBA_Last", miner_name).set(
            hval.get("bba_last_val", 0)
        )
        HBBFT_PERF.labels("hbbft_perf", "Seen_Last", miner_name).set(
    ...
    for line in [x.rstrip("\r\n") for x in results]:
        ...
        if len(c) == 10:
            ...
            if miner_name == val_name:
                ...
                LEDGER_PENALTY.labels("ledger_penalties", "dkg", miner_name).set(
                    dkg_penalty_val
                )
                LEDGER_PENALTY.labels(
                    "ledger_penalties", "performance", miner_name
                ).set(performance_penalty_val)
                LEDGER_PENALTY.labels("ledger_penalties", "total", miner_name).set(
                    total_penalty_val
                )
